...
int InitGL(GLvoid) // All Setup For OpenGL Goes Here
{
	...
	float fogColor[4] = {1.0f, 1.0f, 1.0f, 1.0f}; // Let's make the Fog Color BLUE
...
int DrawGLScene(GLvoid)
{
	...
	else if (brsw)
	{
		...
		if ((showinfo & 1) == 1)
		{
			...
			glVertex3f(255.0f, 0.0f, 0.0f);
			glTexCoord2f(1.0f, 1.0f);
			glVertex3f(255.0f, 255.0f, 0.0f);
		...
		if ((showinfo & 2) == 2)
		{
			...
			glPrint(20, j++ * 12, "Map Name: %s", rsw.mapname);
			j++;
			...
			glPrint(25, j++ * 12, "Models Drawn:  %d", rsw.modelsdrawn);
			j++;

			glPrint(20, j++ * 12, "Object #%d", rsw.selected + 1);
			j++;

			glPrint(25, j++ * 12, "Uses Model #%d", rsw.SelectedModel()->model + 1);
			j++;
			...
			glPrint(25, j++ * 12, "Bounds:");
			glPrint(25, j++ * 12, "Min:  %.2f %.2f %.2f", selectedobject->box.min[0], selectedobject->box.min[1], selectedobject->box.min[2]);
			glPrint(25, j++ * 12, "Max:  %.2f %.2f %.2f", selectedobject->box.max[0], selectedobject->box.max[1], selectedobject->box.max[2]);
			...
			{
			case 0:
				glPrint(360, j++ * 12, "Edit Translation");
				break;
			case 1:
				glPrint(360, j++ * 12, "Edit Rotation");
				break;
			case 2:
				glPrint(360, j++ * 12, "Edit Scaling");
				break;
			}
			...
			{
			case TRUE:
				glPrint(360, j++ * 12, "Culling is ENABLED");
				break;
			case FALSE:
				glPrint(360, j++ * 12, "Culling is DISABLED");
				break;
			}
			...
			{
			case 0:
				glPrint(358, j++ * 12 - 2, "Edit Translation");
				break;
			case 1:
				glPrint(358, j++ * 12 - 2, "Edit Rotation");
				break;
			case 2:
				glPrint(358, j++ * 12 - 2, "Edit Scaling");
				break;
			}
			...
			{
			case TRUE:
				glPrint(358, j++ * 12 - 2, "Culling is ENABLED");
				break;
			case FALSE:
				glPrint(358, j++ * 12 - 2, "Culling is DISABLED");
				break;
			}
		...
		if (showhelp > 0)
		{
			...
			*/
			...
			int j = 2;
			glPrint(offsetx + 20, offsety + j++ * 12, "            - Help Window -");
			j++;
			...
			case 1:
				...
				glPrint(offsetx + 20, offsety + j++ * 12, "F4            Save RSW to file");
				j++;
				glPrint(offsetx + 20, offsety + j++ * 12, "F5            Toggle frustum culling");
				j++;
				glPrint(offsetx + 20, offsety + j++ * 12, "ESC           Exit");
				glPrint(offsetx + 20, offsety + j++ * 12, "CTRL + Enter  Fullscreen on/off");
				j++;
				glPrint(offsetx + 20, offsety + j++ * 12, "Space         Camera Movement on/off");
				j++;
				...
				glPrint(offsetx + 20, offsety + j++ * 12, "RIGHT/D       Pan camera right");
				j++;

				glPrint(offsetx + 20, offsety + j++ * 12, "Mouse         Change Camera angle");
				j++;
				glPrint(offsetx + 20, offsety + j++ * 12, "Left Button   Pick object");
				j++;

				j = 32;
				glPrint(offsetx + 20, offsety + j++ * 12, "                           [F1] Next Page");
				j++;
			...
			case 2:
				...
				glPrint(offsetx + 20, offsety + j++ * 12, "3             Scale mode");
				j++;
				...
				glPrint(offsetx + 20, offsety + j++ * 12, "O/L           edit z value");
				j++;

				glPrint(offsetx + 20, offsety + j++ * 12, "[/]           change model used");
				j++;

				glPrint(offsetx + 20, offsety + j++ * 12, "TAB           toggle screen info/map");
				glPrint(offsetx + 20, offsety + j++ * 12, "Numpad+/-     adjust fog density");
				glPrint(offsetx + 20, offsety + j++ * 12, "INS/DEL       adjust water height");
				j++;

				j = 32;
				glPrint(offsetx + 20, offsety + j++ * 12, "                           [F1] Exit Help");
				j++;
...
BOOL CreateGLWindow(char *title, int width, int height, int bits, bool fullscreenflag)
{
	...
	/* Drain the message queue before the blocking load so Windows doesn't
	   mark the window as "Not Responding" before InitGL finishes. */
	{
		MSG m;
				   ...
				   int nCmdShow)			// Window Show State
{
	...
	if (!fp)
	{
		MessageBox(NULL, "Can't open ragcam.arg", "Error", 0);
		return 0;
	}
	...
	if (!ragnapath)
	{
		MessageBox(NULL, "Can't read ragcam.arg", "Error", 0);
		return 0;
	}
	...
	if (strstr(lpCmdLine, "-extract") != NULL)
		bextract = TRUE;
		...
		// if (!done) // If There Are No Messages
		{
				...
				if (keys[VK_ESCAPE]) // Was ESC Pressed?
				{
					...
					else
					{
						while (ShowCursor(TRUE) < 0)
							;
						if (MessageBox(hWnd, "Are you sure you want to quit?", "RagCam", MB_YESNO + MB_ICONQUESTION) == IDYES)
			...
			if (keys[VK_U])
			{
				...
				if (brsw)
					...
					{
					case 0:
						rsw.SelectedModel()->position.x += 1 * inc;
						break;
					case 1:
						rsw.SelectedModel()->position.rx += 1 * inc;
						break;
					case 2:
						rsw.SelectedModel()->position.sx += 0.01 * inc;
						break;
					case 3:
						g_LightPosition[0] += 1;
						break;
			...
			if (keys[VK_I])
			{
				...
				if (brsw)
					...
					{
					case 0:
						rsw.SelectedModel()->position.y += 1 * inc;
						break;
					case 1:
						rsw.SelectedModel()->position.ry += 1 * inc;
						break;
					case 2:
						rsw.SelectedModel()->position.sy += 0.01 * inc;
						break;
					case 3:
						g_LightPosition[1] += 1;
						break;
			...
			if (keys[VK_O])
			{
				...
				if (brsw)
					...
					{
					case 0:
						rsw.SelectedModel()->position.z += 1 * inc;
						break;
					case 1:
						rsw.SelectedModel()->position.rz += 1 * inc;
						break;
					case 2:
						rsw.SelectedModel()->position.sz += 0.01 * inc;
						break;
					case 3:
						g_LightPosition[2] += 1;
						break;
			...
			if (keys[VK_L])
			{
				...
				if (brsw)
					...
					{
					case 0:
						rsw.SelectedModel()->position.z--;
						break;
					case 1:
						rsw.SelectedModel()->position.rz--;
						break;
					case 2:
						rsw.SelectedModel()->position.sz -= 0.01;
						break;
					case 3:
						g_LightPosition[2] -= 1;
						break;
			...
			if (keys[VK_RSQUAREBRACKET])
			{
				...
				if (brsw)
				{
					...
					tempmodel = rsw.SelectedModel()->model;
					tempmodel++;
					if (tempmodel > rsw.numrealmodels() - 1)
						tempmodel = rsw.numrealmodels() - 1;
					rsw.SelectedModel()->model = tempmodel;
				}
			}
		}

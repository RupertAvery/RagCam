...
/* EUC-KR bytes for "유저인터페이스" (User Interface) — the GRF folder that holds map thumbnails.
   Stored as hex escapes so the path is correct regardless of source-file encoding. */
#define RO_UI_FOLDER "\xC0\xAF\xC0\xFA\xC0\xCE\xC5\xCD\xC6\xE4\xC0\xCC\xBD\xBA"
#define RO_MAP_THUMB_PATH "data\\texture\\" RO_UI_FOLDER "\\map\\"
...
bool RSW::GRFLoad(Grf *grffile, char *filename, HWND window, bool extract)
{
	...
	sprintf(mapfile, RO_MAP_THUMB_PATH "%s.bmp", mapname);
...
void RSW::Display(float waterlevel, CFrustum *g_Frustum)
{
	...
	for (int i = 0; i < nmodels; i++)
	{
		...
		if (bexist[models[i].model])
		{
			...
			// realmodels[models[i].model].DrawAxes(models[i].position);

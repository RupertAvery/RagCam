...
extern "C"
{
	...
	/* Takes the first 8 bytes of array p and converts it into a 64-bit integer */
	static unsigned long long
	getlonglong(unsigned char *p)
	{
		return (unsigned long long)getlong(p) | ((unsigned long long)getlong(p + 4) << 32);
	}
	...
	grf_open(const char *fname, GrfError *error)
	{
		...
		if (strncmp((char *)grf_header, "Master of Magic", 15) != 0 &&
			strncmp((char *)grf_header, "Event Horizon", 13) != 0)
		{
		...
		/* Now seek to the beginning of the file list header.
		   Version 3 uses a 64-bit offset and re-seeks inside its own block. */
		if (grf->version != 3)
		{
			...
			for (entry = 0, index = 0, offset = 0; entry < filelist_entries; entry++)
			{
				...
				if (type != 0)
				{
					...
					if ((ext = strrchr((const char *)name, '.')) != NULL)
					{
						...
						if (strcmp(ext, ".gnd") != 0 && strcmp(ext, ".gat") != 0 && strcmp(ext, ".act") != 0 && strcmp(ext, ".str") != 0)
						{
							unsigned long i;
		...
		else if (grf->version == 3)
		{
			/* Version 3 differences from version 2:
			   - File table offset in the main header is int64 (8 bytes at 0x1E)
			   - Table prefix has 4 bytes of padding before the two size fields
			   - File count at 0x26 is stored directly (no seed/+7 adjustment)
			   - Per-entry file offset is int64 (8 bytes), stride is +22 not +18
			*/
			unsigned long long table_offset;
			unsigned char size_header[8];
			...
			/* Seek using the 64-bit offset stored at header+0x1E */
			table_offset = getlonglong(grf_header + 0x1e);
			if (_fseeki64(f, 46 + (long long)table_offset, SEEK_SET) != 0)
			{
				if (error)
					*error = GE_CORRUPTED;
				free(grf);
			...
			/* Skip 4 bytes of padding before the size fields */
			fread(padding, 1, 4, f);
			...
			if (compressed_size > (uLongf)(grf_size - _ftelli64(f)))
			{
				fclose(f);
				if (error)
					*error = GE_CORRUPTED;
			...
			/* File count is stored directly in v3, not encoded with seed+7 */
			grf->nfiles = getlong(grf_header + 0x26);
			debug("nfiles: %ld\n"
				  "Allocating %d bytes of memory for file list structure.\n",
				  grf->nfiles,
				  sizeof(GrfFile) * grf->nfiles);
			...
			for (entry = 0, offset = 0; entry < grf->nfiles; entry++)
			{
				...
				if (type == 1 || type == 3 || type == 5)
				{
					...
					unsigned long long pos; /* 64-bit in v3 */
					long cycle;

					compressed_len = getlong(filelist_data + ofs2);
					compressed_len_aligned = getlong(filelist_data + ofs2 + 4);
					real_len = getlong(filelist_data + ofs2 + 8);
					pos = getlonglong(filelist_data + ofs2 + 13) + 0x2e;
				...
				/* v3 entry: null-terminator + 21 bytes metadata (pos is 8 bytes) */
				offset += strlen((char *)(filelist_data + offset)) + 22;
			}
	...
	grf_find(Grf *grf, char *fname, unsigned long *index)
	{
		...
		{
			if (_stricmp(grf->files[i].name, fname) == 0)
			{
				if (index)
					*index = i;
				return &(grf->files[i]);
	...
	grf_index_get(Grf *grf, unsigned long index, unsigned long *size, GrfError *error)
	{
		...
		_fseeki64(grf->f, (__int64)file->pos, SEEK_SET);

...
/* EUC-KR bytes for "워터" (water) — the GRF folder for water textures. */
#define RO_WATER_FOLDER "\xBF\xF6\xC5\xCD"
#include "3DMath.h"
...
void GND::LoadWater(char *ragnapath, int type)
{
	...
	texname = (char *)malloc(128);

	for (i = 0; i < 32; i++)
	{
		sprintf(texname, "data\\texture\\" RO_WATER_FOLDER "\\water%d%02d.bmp", type, i);
		// sprintf(texname,"%sdata\\texture\\����\\water%d%02d.jpg",ragnapath, type, i);
		LoadTextureAlpha(ragnapath, texname, watertex[i], 192);
		// BuildTexture (texname, watertex[i]);
	...
	for (i = 0; i < sizeY; i++)
		for (j = 0; j < sizeX; j++)
		{
			...
			if (g_Frustum->CubeInFrustum(j * L, i * L, h1, L * 2))
			{
				...
				vTriangle[2] = MatrixMultVect3f(Mat, j * L, (i + 1) * L, h2);
				vTriangle[1] = MatrixMultVect3f(Mat, (j + 1) * L, (i + 1) * L, h3);
		...
		for (j = 0; j < sizeX; j++)
		{
			...
			if (g_Frustum->CubeInFrustum(j * L, i * L, cubes[i * sizeX + j].y1, L * 2))
			{
				...
				if (tileotherside != -1)
				{
					...
					vTriangle[2] = MatrixMultVect3f(Mat, (j + 1) * L, (i + 1) * L, cubes[(i + 1) * sizeX + j].y1);
					vTriangle[1] = MatrixMultVect3f(Mat, (j + 1) * L, (i + 1) * L, cubes[i * sizeX + j].y4);
				...
				if (tilesup != -1)
				{
					...
					vTriangle[1] = MatrixMultVect3f(Mat, (j + 1) * L, i * L, cubes[i * sizeX + j].y2);
					vTriangle[2] = MatrixMultVect3f(Mat, j * L, (i + 1) * L, cubes[(i + 1) * sizeX + j].y3);

					vNormal = Normal(vTriangle);

					// ugly hack...
					// if (vNormal.y < vTriangle[0].y) vNormal.y = - vNormal.y;

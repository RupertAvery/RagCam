...
void RSM_Mesh::Display(bounding_box_t *box, ro_transf_t *ptransf)
{
	...
	if (nframes)
	{
		...
		t = ((GLfloat)(nstep - frames[current].time)) / ((GLfloat)(frames[next].time - frames[current].time));
	...
	for (i = 0; i < nall_faces; i++)
	{
		...
		t = &tex_vtx[all_faces[i].t[0]];
		glTexCoord2f((*t)[1], (*t)[2]);
		v = &pos_vtx[all_faces[i].v[0]];
		glVertex3f((*v)[0], (*v)[1], (*v)[2]);

		t = &tex_vtx[all_faces[i].t[2]];
		glTexCoord2f((*t)[1], (*t)[2]);
		v = &pos_vtx[all_faces[i].v[2]];
		glVertex3f((*v)[0], (*v)[1], (*v)[2]);

		t = &tex_vtx[all_faces[i].t[1]];
		glTexCoord2f((*t)[1], (*t)[2]);
		v = &pos_vtx[all_faces[i].v[1]];
		glVertex3f((*v)[0], (*v)[1], (*v)[2]);
	}

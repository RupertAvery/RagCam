...
*/
		...
		vin[0] = 0;	vin[1] = 0;	vin[2] = 0;
		MatrixMultVect(Mat, vin, vcenter);

		vin[0] = 10; vin[1] = 0; vin[2] = 0;
		MatrixMultVect(Mat, vin, vcenterx);

		vin[0] = 0; vin[1] = 10; vin[2] = 0;
		MatrixMultVect(Mat, vin, vcentery);

		vin[0] = 0;	vin[1] = 0;	vin[2] = 10;
...
void RSM::Display(ro_position_t pos)
{
	...
	if (  g_Frustum.PointInFrustum(vBox[0].x,vBox[0].y,vBox[0].z)
		| g_Frustum.PointInFrustum(vBox[1].x,vBox[1].y,vBox[1].z)
		| g_Frustum.PointInFrustum(vBox[2].x,vBox[2].y,vBox[2].z)
		| g_Frustum.PointInFrustum(vBox[3].x,vBox[3].y,vBox[3].z)
		| g_Frustum.PointInFrustum(vBox[4].x,vBox[4].y,vBox[4].z)
		| g_Frustum.PointInFrustum(vBox[5].x,vBox[5].y,vBox[5].z)
		| g_Frustum.PointInFrustum(vBox[6].x,vBox[6].y,vBox[6].z)
		| g_Frustum.PointInFrustum(vBox[7].x,vBox[7].y,vBox[7].z)
		)
